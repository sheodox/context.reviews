...
import { connectedUsers, connectedWebsockets } from '../metrics.js';
	...
	return new Promise((resolve, reject) => {
		...
		if (typeof sid === 'string') {
			sessionStore.get(sid, (err, session) => {
				...
				err || !(session && session.passport && session.passport.user) ? reject(null) : resolve(session.passport.user);
			});
		} else {
