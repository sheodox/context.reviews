...
import { URL } from 'url';
import { lookupsCacheHit, lookups, lookupsNoResults, lookupTime } from '../metrics.js';
import { lookupLogger } from './logger.js';
import type { Definition, SearchResults, JapaneseForm, JishoExternalLink } from '../../shared/types/definitions';
...
export class JishoSearch {
	...
	static async search(searchText: string): Promise<SearchResults> {
		...
		const searchResultsUrl = (word: string) => `https://jisho.org/search/${encodeURIComponent(word)}`,
			...
			result: JishoResponse = JSON.parse(
				await fetch(`https://jisho.org/api/v1/search/words?keyword=${encodeURIComponent(searchText)}`)
			);
		lookupEnd();

		if (result.meta.status !== 200) {
			lookupLogger.error(`Error looking up "${searchText}"`, {
				result,
			});
		} else {
			//success
			const definitions = result.data.map((res): Definition => {
				...
				tags = tags.concat([...res.jlpt, ...res.tags]);
