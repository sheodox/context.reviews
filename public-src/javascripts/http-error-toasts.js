...
export function getDefaultHttpErrorMessage(res) {
 ...
 */
export async function createHttpErrorToast(message, response, parsedBody) {
	let error, requestId;
	try {
	...
	const statusDefaults = statusMessageMap.get(response.status) || {message: `An unknown error occurred.`, showTechnicalDetails: true}
